...
### ajjc: https://www.quandl.com/api/v3/datatables/WIKI/PRICES?qopts.export=true&api_key=YOURAPIKEY
QUANDL_DATA_URL = 'https://www.quandl.com/api/v3/datatables/SHARADAR/'
QUANDL_DATA_URL_SEP = (
...
#Sharadar Equity+Funds Bundle

@bundles.register('sharadar-eqfd')
def sharadar_equity_plus_fund_bundle(environ,
                  asset_db_writer,
                  minute_bar_writer,
                  daily_bar_writer,
                  adjustment_writer,
    ...
    raw_data = raw_data_SEP.append(raw_data_SFP)
...
def format_metadata_url_SEP(api_key):
    """ Build the query URL for Quandl Prices metadata.
    """
    query_params = [('api_key', api_key), ('qopts.export', 'true')]

    return (
        QUANDL_DATA_URL_SEP + urlencode(query_params)
    )

def format_metadata_url_SFP(api_key):
    """ Build the query URL for Quandl Prices metadata.
    """
    query_params = [('api_key', api_key), ('qopts.export', 'true')]

    return (
        QUANDL_DATA_URL_SFP + urlencode(query_params)
    )
...
def format_metadata_url(api_key):
    ...
    """
    query_params = [('api_key', api_key), ('qopts.export', 'true')]
    quandl_url_str = QUANDL_DATA_URL + tbl_name + '.csv?' + urlencode(query_params)
    return (quandl_url_str)
...
def fetch_data_table(api_key,
                     show_progress,
                     retries, tbl_name='SEP'):
    for _ in range(retries):
        try:
            if show_progress:
                log.info('Downloading Sharadar S{E,F}P metadata.')

            if tbl_name == 'SEP':
                metadata = pd.read_csv(
                    format_metadata_url_SEP(api_key)
                )
            elif tbl_name == 'SFP':
                metadata = pd.read_csv(
                    format_metadata_url_SFP(api_key)
                )
            else:
                 log.info('Tbl name(SEP,SFP) not in list.')
            ...
            if show_progress:
                raw_file = download_with_progress(
                    ...
                    label="Downloading table from Quandl Sharadar"
                )
            else:
...
register_calendar_alias('sharadar-eqfd', "NYSE")
